...
const RegisterForm = (props) => {
  ...
  const onSubmit = ({
    ...
    if (password !== confirmPassword) {
      return setRegisterErrorMessage("Password don't match");
    }
    ...
    register(
      ...
      (err) => {
        ...
        setIsLoading(false);
        if (err === undefined)
          setRegisterErrorMessage("Please Check your connection");
      }
    );
  };
  if (redirect)
    return <Redirect to={{ pathname: redirect, reg: "registered" }} />;
  return (
        ...
        <div className="w-10/12 px-10">
          ...
          {registerErrorMessage && (
            <Alert
              ...
              callback={() => setRegisterErrorMessage()}
            />
          )}
